...
def _set_average(trainer, trainers, machine, n_samples, trainer_type):
  ...
  **Parameters:**
  ...
  n_samples : int
    Total number of samples used in training
  """
  ...
  if trainer_type == "KMeansTrainer":
    ...
    trainer.average_min_distance /= n_samples
...
def _parallel_e_step(args):
  ...
  """
  trainer, machine, data, trainer_type = args
  trainer.e_step(machine, data)
  if trainer_type == "KMeansTrainer":
    trainer.average_min_distance = trainer.average_min_distance * len(data)
  ...
  **Parameters**:
  ...
  def _e_step(trainer, machine, data):
    ...
    if pool is None:
      ...
      pool.map(_parallel_e_step, zip(trainers, machines, split_data, [trainer_type]*len(trainers)))
      # update the trainer with the data of the other trainers
      _set_average(trainer, trainers, machine, len(data), trainer_type)
  ...
  for i in range(max_iterations):
    ...
    if hasattr(trainer,"compute_likelihood"):
      ...
      if convergence_threshold is not None and convergence_value <= convergence_threshold:
        break
  if hasattr(trainer,"finalize") and max_iterations != 0:
